				...
				if (have_posts()):
					...
					<div class="wm__inner">
						...
						<div class="wm__postmeta">
					        <strong>Terbit</strong> <i class="icon-wm-clock"></i> <?php printf(__('%s', 'masjid'), get_the_time('j F Y')); ?> | <strong>Oleh</strong> : <?php the_author(); ?> | Kat : <?php
										    $terms = get_the_terms( $post->ID , 'kat-tausiyah' );
							                if ( $terms != null ){
									        	foreach( $terms as $termkat ) {
										        	$term_link = get_term_link( $termkat, 'kat-tausiyah');
										        	echo '<a href="' . esc_url( $term_link ) . '">' . esc_html( $termkat->name ) . '</a> ';
										        	unset($termkat); 
									        	} 
								        	} 
										?>
